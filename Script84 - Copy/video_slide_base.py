        ...
        if "video" in slide:
            ...
            if self.current_video_clip is None and not self.video_finished:
                if os.path.exists(video_path):
                    # Create FFPyPlayer instance
                    # ff_opts={'paused': True} allows us to control start
                    self.current_video_clip = MediaPlayer(video_path)
                    self.video_start_time = time.time()
                    
                    # Handle separate audio if provided
                    if video_data.get("audio_path"):
                         self.main_app.audio_manager.play_sound(video_data["audio_path"], audio_type='video_audio')
                         # Mute video internal audio if using separate file
                         self.current_video_clip.set_volume(0.0) 
                else:
                    self.video_finished = True

            # Play & Render
            if self.current_video_clip and not self.video_finished:
                frame, val = self.current_video_clip.get_frame()
                
                if val == 'eof':
                    self.video_finished = True
                    self.current_video_clip.close_player()
                    self.current_video_clip = None
                elif frame is not None:
                    # Convert FFPyPlayer frame to Pygame Surface
                    img, t = frame
                    w, h = img.get_size()
                    data = img.to_bytearray()[0]
                    
                    # Create surface
                    video_surf = pygame.image.frombuffer(data, (w, h), "RGB")
