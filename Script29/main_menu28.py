...
class QuizApp:
    def __init__(self, selected_profile):
        ...
        # ✅ FIX: Better state management for audio context
        self.audio_context = {
            'type': 'bg_music',  # 'bg_music', 'video_audio', or 'narration'
            'path': None,
            'was_playing': False
        }
    ...
    def play_bg_music(self):
       ...
       # ✅ NEW: Helper methods for audio context management
    def save_audio_context(self):
        """Save current audio state before opening profile manager."""
        self.audio_context['was_playing'] = pygame.mixer.music.get_busy()
        
        # Determine what type of audio is playing
        # This should be set by the calling screen
        if not hasattr(self, '_current_audio_type'):
            self._current_audio_type = 'bg_music'
            
        self.audio_context['type'] = self._current_audio_type
        self.audio_context['path'] = getattr(self, '_current_audio_path', None)

    def restore_audio_context(self):
        """Restore audio state after returning from profile manager."""
        if not self.audio_context['was_playing']:
            return  # Don't resume if nothing was playing
            
        try:
            if self.audio_context['type'] == 'video_audio' and self.audio_context['path']:
                # Resume video audio
                pygame.mixer.music.load(self.audio_context['path'])
                pygame.mixer.music.play()
            elif self.audio_context['type'] == 'narration' and self.audio_context['path']:
                # Resume narration
                pygame.mixer.music.load(self.audio_context['path'])
                pygame.mixer.music.play()
            elif self.audio_context['type'] == 'bg_music':
            ...
            # ✅ CRITICAL: Clear the context after use
            self.audio_context['path'] = None
            self._current_audio_type = 'bg_music'
            self._current_audio_path = None

    # ✅ UPDATED: Simplified profile manager method
    def open_profile_manager(self):
        """Open profile manager with proper audio state management."""
        # Save current audio state
        self.save_audio_context()
        
        # Stop all sounds before opening profile manager
        pygame.mixer.music.stop()
        for i in range(pygame.mixer.get_num_channels()):
            pygame.mixer.Channel(i).stop()

        # Open profile manager
        updated = run_profile_manage_screen(
            screen, 
            config.SCREEN_WIDTH, 
    ...
    def open_profile_manager(self):
        ...
        if updated:
            if updated.get("_deleted"):
                # Profile deleted - select new one
                self.selected_profile = run_profile_screen(
                    screen, 
                    config.SCREEN_WIDTH, 
                    config.SCREEN_HEIGHT
                )
                # After selecting new profile, always play main BG music
                self.play_bg_music()
            else:
                # Profile updated - refresh avatar
                self.selected_profile.update(updated)
                avatar_path = os.path.join(
                    config.AVATAR_PROFILES_PATH, 
                    self.selected_profile["avatar"]
                )
                self.profile_icon = load_image(
                    avatar_path, 
                    config.SCREEN_WIDTH * 0.04, 
                    config.SCREEN_WIDTH * 0.04
                )
                self.profile_rect = self.profile_icon.get_rect(
                    topright=(config.SCREEN_WIDTH - 30, 30)
                )
                
                # ✅ Restore audio based on saved context
                self.restore_audio_context()
