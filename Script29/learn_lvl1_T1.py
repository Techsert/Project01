        ...
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                ...
                if main_app.exit_rect.collidepoint(e.pos):
                    ...
                    # ✅ Simplified: Videos have no sound, only external audio files
                    ...
                    # Stop sounds and open profile manager
                    stop_sounds()
                    main_app.open_profile_manager()

                    # ✅ FIX: Reset video state when returning
                    if "video" in current_slide_data:
                        current_video_clip = None
                        video_start_time = 0
                        video_finished = False
                        last_rendered_video_frame_surface = None
                    # Audio will be automatically restored by restore_audio_context()
                    ...
                    # ✅ FIX: Clear audio context when leaving
                    main_app._current_audio_type = 'bg_music'
                    main_app._current_audio_path = None
