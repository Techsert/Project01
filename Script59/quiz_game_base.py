...
class QuizGameBase:
    ...
    def __init__(self, level=1, difficulty="easy"):
        ...
        self.main_app = None  # Will be set by run_quiz_lvl* functions
    ...
    # ============================================================
    # ADD THIS TO quiz_game_base.py - In the __init__ method
    # ============================================================

    # ✅ NEW: Load feedback icons for results screen
    def load_feedback_icon(path, size=(50, 50), fallback_color=(200, 200, 200)):
        """Load feedback icon with fallback"""
        ...
        surf = pygame.Surface(size, pygame.SRCALPHA)
        pygame.draw.circle(surf, fallback_color + (220,), (size[0]//2, size[1]//2), min(size)//2 - 2)
        return surf

    # ✅ In __init__ method of QuizGameBase, add after other _load methods:
    def __init__(self, level=1, difficulty="easy"):
        # ... existing code ...
        
        # ✅ NEW: Load feedback icons
    ...
    def _load_feedback_icons(self):
        ...
        self.party_icon = load_feedback_icon(
            os.path.join(ICONS_PATH, "party_icon.png"),
            size=(50, 50),
            fallback_color=(0, 255, 0)  # Green for excellent
        )
        
        self.diamond_icon = load_feedback_icon(
            os.path.join(ICONS_PATH, "diamond_icon.png"),
            size=(50, 50),
            fallback_color=(255, 215, 0)  # Gold for good
        )
        
        self.tryagain_icon = load_feedback_icon(
            os.path.join(ICONS_PATH, "tryagain_icon.png"),
            size=(50, 50),
            fallback_color=(255, 100, 100)  # Red for try again
        )


    # ============================================================
    # UPDATE THE draw_results METHOD in QuizGameBase
    # ============================================================
    def draw_results(self):
        """Draw results screen with IMAGE ICONS instead of emojis"""
        ...
        if self.current_profile and hasattr(self, '_quiz_recorded') and not self._quiz_recorded:
        ...
        score_text = render_arabic_to_surface(f"{score_percent}%", ARABIC_FONT_PATH or "", 120, color=(255,215,0), bold=True)
        ...
        if score_percent >= 90:
            msg = "ممتاز!"  # Removed emoji
            msg_color = (0, 255, 0)
            feedback_icon = self.party_icon
        elif score_percent >= 70:
            msg = "جيد!"  # Removed emoji
            msg_color = (255, 215, 0)
            feedback_icon = self.diamond_icon
        else:
            msg = "حاول مرة أخرى!"  # Removed emoji
            msg_color = (255, 100, 100)
            feedback_icon = self.tryagain_icon
        
        # Draw message text
        msg_surf = render_arabic_to_surface(msg, ARABIC_FONT_PATH or "", 50, color=msg_color, bold=True)
        ...
        txt_correct = render_arabic_to_surface(str(correct), "arial" or "", 200, color=(0,255,0))
        txt_wrong = render_arabic_to_surface(str(wrong), "arial" or "", 200, color=(255,0,0))
