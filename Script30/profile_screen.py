...
##        screen.blit(avatars_title, (SCREEN_WIDTH // 2 - avatars_title.get_width() // 2, 400))
        ...
        AVATAR_SIZE = 120
        SPACING_X = 50
        y = int(SCREEN_HEIGHT * 0.7)  # vertical position of the row
...
def run_profile_manage_screen(screen, SCREEN_WIDTH_PARAM, SCREEN_HEIGHT_PARAM, profile): # Use distinct param names
    ...
    def draw():
        ...
        TITLE_X = int(SCREEN_WIDTH * PROFILE_TITLE_X_RATIO) # Use config
        TITLE_Y = int(SCREEN_HEIGHT * PROFILE_TITLE_Y_RATIO) # Use config
        ...
        avatar_size = int(SCREEN_WIDTH * AVATAR_SIZE_RATIO) # Use config
        x = int(SCREEN_WIDTH * AVATAR_START_X_RATIO) # Use config
        y = int(SCREEN_HEIGHT * AVATAR_START_Y_RATIO) # Use config
        ...
        for a in AVAILABLE_AVATARS:
            ...
            avatar_images[a + "_rect"] = r
            x += int(SCREEN_WIDTH * (AVATAR_SIZE_RATIO + AVATAR_GAP_RATIO)) # Use config  # move to next slot
        ...
        cols = 7
        card_width = int(SCREEN_WIDTH * 0.05) # Use config
