...
class PracticeBase:
    ...
    def draw_tracing(self):
        ...
        # ==========================================
        # 🛠️ TEMPORARY DEBUG GRID (DELETE LATER)
        # ==========================================
        # 1. Draw light gray lines every 50 pixels
        for x in range(0, self.W, 50):
            pygame.draw.line(self.screen, (200, 200, 200), (x, 0), (x, self.H))
        for y in range(0, self.H, 50):
            pygame.draw.line(self.screen, (200, 200, 200), (0, y), (self.W, y))

        # 2. Draw RED lines for the Center (X=0, Y=0 offset reference)
        pygame.draw.line(self.screen, (255, 0, 0), (self.W // 2, 0), (self.W // 2, self.H), 2)
        pygame.draw.line(self.screen, (255, 0, 0), (0, self.H // 2), (self.W, self.H // 2), 2)

        # 3. Show Mouse Coordinates at top-left
        mx, my = pygame.mouse.get_pos()
        try:
            font = pygame.font.SysFont("Arial", 40, bold=True)
            # Shows exact X,Y and the Offset from center (useful for your item positioning)
            coord_text = f"Mouse: {mx}, {my} | Center Offset: {mx - self.W//2}, {my - self.H//2}"
            
            # Draw text with black background so it's readable
            surf = font.render(coord_text, True, (255, 0, 0), (255, 255, 255))
        ...
        for event in pygame.event.get():
            ...
            # ✅ Handle mouse motion for dragging and tracing
                ...
                if self.trace_path and q.get("letter_offset"):
                    ...
                    # ✅ NEW: Check if user is staying close to the expected LINE segment
                    if self.last_matched_index < len(self.trace_path):
                        current_guide = self.trace_path[self.last_matched_index]
                        adjusted_current = (
                            current_guide[0] + letter_offset[0],
                            current_guide[1] + letter_offset[1]
                        )
                        
                        # If there's a previous point, check distance to the LINE between them
                        if self.last_matched_index > 0:
                            prev_guide = self.trace_path[self.last_matched_index - 1]
                            adjusted_prev = (
                            ...
                            # ✅ If too far from the line, STOP and reset
                            if line_dist > self.trace_tolerance:
                                print(f"❌ TOO FAR FROM LINE: {line_dist:.1f} > {self.trace_tolerance}")
                                self._play_tryagain_sound()
                                self.tracing_active = False
                                self.trace_points = []
                                self.trace_progress = 0.0
                        ...
                        # Check if reached the next guide point
