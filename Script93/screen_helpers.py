...
def back_confirm_popup(screen, font_loader):
    """
    Display Arabic back/exit confirmation popup.
    Returns True if user clicks Yes, False if No.
    
    Args:
        screen: pygame.Surface
        font_loader: function(size, bold=False) -> pygame.font.Font
    """
    SCREEN_WIDTH, SCREEN_HEIGHT = screen.get_size()
    ...
    while show_popup:
        ...
        pygame.draw.rect(screen, (0, 0, 0), box_rect, 4, border_radius=20)
        
        # Draw message
        popup_font = font_loader(int(SCREEN_HEIGHT * 0.04), bold=True)
        msg_surf = render_text("هل تريد الخروج من اللعبة؟", popup_font, (0, 0, 0))
        msg_rect = msg_surf.get_rect(center=(SCREEN_WIDTH//2, SCREEN_HEIGHT//2 - 50))
        screen.blit(msg_surf, msg_rect)
        
        # Draw Yes/No buttons
        btn_w, btn_h = 200, 80
        yes_rect = pygame.Rect(SCREEN_WIDTH//2 - 220, SCREEN_HEIGHT//2 + 40, btn_w, btn_h)
        no_rect = pygame.Rect(SCREEN_WIDTH//2 + 20, SCREEN_HEIGHT//2 + 40, btn_w, btn_h)
        
        pygame.draw.rect(screen, (200, 50, 50), yes_rect, border_radius=15)
        pygame.draw.rect(screen, (50, 150, 50), no_rect, border_radius=15)
        
        btn_font = font_loader(int(SCREEN_HEIGHT * 0.03), bold=True)
        yes_text = render_text("نعم", btn_font, (255, 255, 255))
        no_text = render_text("لا", btn_font, (255, 255, 255))
