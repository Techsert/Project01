# video_slide_base.py - REFACTORED TO USE AUDIO_MANAGER
import pygame
import sys
import time
import os
...
from screen_helpers import confirm_popup, load_font  # ✅ IMPORT CONFIRM_POPUP
from profile_system import mark_learning_complete
...
class NavigableVideoSlidePlayer(VideoSlidePlayer):
    ...
    def check_and_mark_completion(self):
        """Check if user completed all slides and mark learning level as complete"""
        if self.slide_index >= len(self.slides) - 1:  # On last slide
            ...
            if profile_name and not hasattr(self, '_learning_recorded'):
                mark_learning_complete(profile_name, self.level)
                self._learning_recorded = True
                print(f"✅ {profile_name} completed Learning Level {self.level}")
