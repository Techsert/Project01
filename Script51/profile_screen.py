...
def run_profile_manage_screen(screen, SCREEN_WIDTH_PARAM, SCREEN_HEIGHT_PARAM, profile):
    ...
    def draw_progress_section():
        ...
        for level in [1, 2, 3]:
            ...
            learn_text = FONT.render(f"{learn_icon} Learning {level}: {learn_status}", True, learn_color)
            quiz_text = FONT.render(f"{quiz_icon} Quiz {level}: {quiz_status}", True, quiz_color)
            
            screen.blit(learn_text, (int(SCREEN_WIDTH * (TEXT_X + 0.02)), int(SCREEN_HEIGHT * y_pos)))
            screen.blit(quiz_text, (int(SCREEN_WIDTH * (TEXT_X + 0.22)), int(SCREEN_HEIGHT * y_pos)))
