...
def draw_quiz_screen(screen, main_app, title, buttons, bg_image, bg_rect, level=1):
    """Draws a quiz topic screen with buttons, profile, exit icons, and lock/unlock indicators."""
    ...
    while running:
        ...
        for i, b in enumerate(buttons):
            # ✅ Check if this quiz level is unlocked (requires matching learning level complete)
            quiz_level = i + 1
            is_unlocked = unlock_status.get("quiz", {}).get(quiz_level, False)
            ...
            # Define text color based on unlock status
            button_text_color = (180, 0, 200) if is_unlocked else (150, 150, 150)

            # Define text position offset from center
            offset_x = 0
            offset_y = 120
        ...
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                ...
                # ✅ Quiz buttons (check if unlocked)
                else:
                    for i, b in enumerate(buttons):
                        if b["rect"].collidepoint(e.pos):
                            ...
                            if is_unlocked:
                                ...
                                msg = f"Complete Learning Level {quiz_level} first to unlock this quiz!"
