...
class QuizApp:
    ...
    def create_new_profile(self, name=None, avatar_file=None):
        profile = {"name": name, "avatar": avatar_file}
        self.profiles.append(profile)
        save_profiles(self.profiles)
        # Stay on profile screen until the user explicitly selects a profile

    # ---------------------------
    # New/updated methods for QuizApp
    # ---------------------------

    def create_profile_screen(self):
        """
        Screen to create a new profile: enter name, enter PIN, select avatar.
        """
        input_name = ""
        input_pin = ""
        avatar_files = os.listdir(AVATAR_PATH)
        avatar_index = 0
        entering_name = True  # True: entering name, False: entering PIN

        creating = True
        while creating:
            screen.fill((255,255,255))
            self.draw_arabic_text("إنشاء ملف جديد", 150, (0,0,0), (SCREEN_WIDTH//2, SCREEN_HEIGHT*0.1), bold=True)
            
            # Name input
            name_rect = pygame.Rect(SCREEN_WIDTH//2 - 200, SCREEN_HEIGHT//3 - 50, 400, 80)
            pygame.draw.rect(screen, (200,200,200), name_rect)
            self.draw_arabic_text(input_name or "أدخل الاسم هنا", 50, (0,0,0), name_rect.center)

            # PIN input
            pin_rect = pygame.Rect(SCREEN_WIDTH//2 - 200, SCREEN_HEIGHT//3 + 100, 400, 80)
            pygame.draw.rect(screen, (200,200,200), pin_rect)
            masked_pin = "*"*len(input_pin)
            self.draw_arabic_text(masked_pin or "أدخل رمز PIN", 50, (0,0,0), pin_rect.center)
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        # entering PIN (numbers only)
                        if event.key == pygame.K_BACKSPACE:
                            input_pin = input_pin[:-1]
                        elif event.unicode.isdigit() and len(input_pin)<4:
                            input_pin += event.unicode
                    # Toggle between name and PIN by pressing Enter
                    ...
                    if confirm_rect.collidepoint(event.pos):
                        # Only allow creation if both name and 4-digit PIN are entered
                        if input_name.strip() != "" and len(input_pin) == 4:
                            profile = {
                                "name": input_name.strip(),
                                "avatar": avatar_files[avatar_index],
                                "pin": input_pin
                            }
                            self.profiles.append(profile)
                            save_profiles(self.profiles)
                            ...
                            # Optional: flash warning text
                            self.draw_arabic_text("يرجى إدخال الاسم و PIN صحيح", 40, (255,0,0), (SCREEN_WIDTH//2, SCREEN_HEIGHT//2 + 300))
    ...
    def verify_pin(self, profile):
        """
        Ask for PIN entry to select existing profile.
        """
        input_pin = ""
        verifying = True
        while verifying:
            screen.fill((255,255,255))
            self.draw_arabic_text(f"أدخل رمز PIN ل {profile['name']}", 150, (0,0,0), (SCREEN_WIDTH//2, SCREEN_HEIGHT//3), bold=True)
            pin_rect = pygame.Rect(SCREEN_WIDTH//2 - 150, SCREEN_HEIGHT//2 - 50, 300, 80)
            pygame.draw.rect(screen, (200,200,200), pin_rect)
            masked_pin = "*"*len(input_pin)
            self.draw_arabic_text(masked_pin or "أدخل رمز PIN", 50, (0,0,0), pin_rect.center)
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_ESCAPE:
                        ...
                        if input_pin == profile["pin"]:
                            ...
                            # Optional: flash warning text
                            self.draw_arabic_text("رمز PIN غير صحيح", 50, (255,0,0), (SCREEN_WIDTH//2, SCREEN_HEIGHT//2 + 150))
                            input_pin = ""
    ...
    def run(self):
        ...
        while self.running:
            ...
            if self.show_profile_screen:
                # Draw profile screen and store rects
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.show_profile_screen:
                        ...
                        for rect, profile in profile_rects:
                            if rect.collidepoint(mouse_pos):
                                self.verify_pin(profile)  # ask user for PIN before entering
                                break

                        # Edit existing profile
                        for rect, profile in edit_rects:
                            if rect.collidepoint(mouse_pos):
                                self.edit_profile(profile)
                                break

                        # Delete existing profile
                        for rect, profile in delete_rects:
                            if rect.collidepoint(mouse_pos):
                        ...
                        if add_rect and add_rect.collidepoint(mouse_pos):
                            self.create_profile_screen()  # user enters name, PIN, avatar
                            break
