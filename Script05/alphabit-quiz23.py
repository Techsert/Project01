...
##            pygame.draw.rect(self.screen, (0,0,0), rect, 1)
    ...
    def draw_results(self):
        ...
        icon_size = 60
        spacing = 10
        start_x = self.W//2 - (total*(icon_size+spacing))//2
        y_correct = int(self.H*0.35)
        y_wrong = int(self.H*0.5)

        # Correct answers - football goals
        for i in range(correct):
            if hasattr(self, "goal_icon") and self.goal_icon:
                self.screen.blit(self.goal_icon, (start_x + i*(icon_size+spacing), y_correct))
            else:
                pygame.draw.circle(self.screen, (0,180,0), (start_x + i*(icon_size+spacing)+icon_size//2, y_correct+icon_size//2), icon_size//2)

        # Wrong answers - football misses
        for i in range(wrong):
            if hasattr(self, "miss_icon") and self.miss_icon:
                self.screen.blit(self.miss_icon, (start_x + (correct+i)*(icon_size+spacing), y_wrong))
            else:
                pygame.draw.circle(self.screen, (180,0,0), (start_x + (correct+i)*(icon_size+spacing)+icon_size//2, y_wrong+icon_size//2), icon_size//2)
        ...
        # Store play again rect for click detection
                 ...
                 # ----------------- QUESTION OPTION CLICK HANDLING -----------------
                    ...
                    if self.state == "results":
                        if "play_again" in self.results_buttons and self.results_buttons["play_again"].collidepoint(event.pos):
                            self.select_difficulty(self.selected_difficulty)  # restart same difficulty
