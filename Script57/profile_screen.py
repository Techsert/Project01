...
def run_profile_manage_screen(screen, SCREEN_WIDTH_PARAM, SCREEN_HEIGHT_PARAM, profile):
    ...
    def draw_progress_section():
        """Draw progress and achievements section"""
        ...
        if completed_learning:
            for i, level in enumerate(sorted(completed_learning)):
                ...
                screen.blit(level_text, (int(SCREEN_WIDTH * (TEXT_X + 0.02)), int(SCREEN_HEIGHT * y_pos)))
        else:
        ...
        if quiz_scores:
            for i, (level, score) in enumerate(sorted(quiz_scores.items())):
                ...
                score_text = FONT.render(f"{icon} Level {level}: {score}% (Best)", True, color)
                screen.blit(score_text, (int(SCREEN_WIDTH * (TEXT_X + 0.02)), int(SCREEN_HEIGHT * y_pos)))
        else:
        ...
        # ✅ Load lock/unlock icons (cached for performance)
        if not hasattr(draw_progress_section, '_lock_icons_loaded'):
            # Locked icon (red lock)
            lock_img_path = os.path.join(config.IMG_MAIN_PATH, "lock_icon.png")
            if os.path.exists(lock_img_path):
                draw_progress_section.lock_icon = pygame.image.load(lock_img_path).convert_alpha()
                draw_progress_section.lock_icon = pygame.transform.smoothscale(draw_progress_section.lock_icon, (30, 30))
            else:
                # Fallback: create red lock surface
                draw_progress_section.lock_icon = pygame.Surface((30, 30), pygame.SRCALPHA)
                pygame.draw.rect(draw_progress_section.lock_icon, (200, 0, 0), (8, 12, 14, 15), border_radius=3)
                pygame.draw.arc(draw_progress_section.lock_icon, (200, 0, 0), (10, 5, 10, 12), 0, 3.14, 3)
            
            # Unlocked icon (green checkmark/unlock)
            unlock_img_path = os.path.join(config.IMG_MAIN_PATH, "unlock_icon.png")
            if os.path.exists(unlock_img_path):
                draw_progress_section.unlock_icon = pygame.image.load(unlock_img_path).convert_alpha()
                draw_progress_section.unlock_icon = pygame.transform.smoothscale(draw_progress_section.unlock_icon, (30, 30))
            else:
                # Fallback: create green checkmark surface
                draw_progress_section.unlock_icon = pygame.Surface((30, 30), pygame.SRCALPHA)
                pygame.draw.line(draw_progress_section.unlock_icon, (0, 200, 0), (8, 15), (13, 22), 4)
                pygame.draw.line(draw_progress_section.unlock_icon, (0, 200, 0), (13, 22), (22, 8), 4)
            
            # Partial unlock icon (yellow/orange)
            partial_img_path = os.path.join(config.IMG_MAIN_PATH, "partial_icon.png")
            if os.path.exists(partial_img_path):
                draw_progress_section.partial_icon = pygame.image.load(partial_img_path).convert_alpha()
                draw_progress_section.partial_icon = pygame.transform.smoothscale(draw_progress_section.partial_icon, (30, 30))
            else:
                # Fallback: create yellow half-circle surface
                draw_progress_section.partial_icon = pygame.Surface((30, 30), pygame.SRCALPHA)
                pygame.draw.circle(draw_progress_section.partial_icon, (220, 180, 0), (15, 15), 12, 3)
                pygame.draw.line(draw_progress_section.partial_icon, (220, 180, 0), (15, 3), (15, 27), 3)
            
            draw_progress_section._lock_icons_loaded = True
        ...
        for level in [1, 2, 3]:
            ...
            # ✅ FIX: Check if ANY topic in the level is unlocked (not all)
            ...
            # Learning status - show if at least one topic is accessible
            if all_topics_unlocked:
                learn_icon_img = draw_progress_section.unlock_icon
                learn_color = (0, 150, 0)
                learn_status = "All Topics Unlocked"
            elif any_topic_unlocked:
                learn_icon_img = draw_progress_section.partial_icon
                ...
                learn_icon_img = draw_progress_section.lock_icon
                learn_color = (150, 0, 0)
                learn_status = "Locked"
            
            # Quiz status
            if quiz_unlocked.get(level, False):
                quiz_icon_img = draw_progress_section.unlock_icon
                quiz_color = (0, 150, 0)
                quiz_status = "Unlocked"
            else:
                quiz_icon_img = draw_progress_section.lock_icon
                quiz_color = (150, 0, 0)
                quiz_status = "Locked"
            
            # ✅ Blit icon images and text
            learn_x = int(SCREEN_WIDTH * (TEXT_X + 0.02))
            learn_y = int(SCREEN_HEIGHT * y_pos)
            
            # Draw learning icon and text
            screen.blit(learn_icon_img, (learn_x, learn_y - 5))
            learn_text = FONT.render(f"Learning {level}: {learn_status}", True, learn_color)
            screen.blit(learn_text, (learn_x + 35, learn_y))
            
            # Draw quiz icon and text
            quiz_x = int(SCREEN_WIDTH * (TEXT_X + 0.22))
            screen.blit(quiz_icon_img, (quiz_x, learn_y - 5))
            quiz_text = FONT.render(f"Quiz {level}: {quiz_status}", True, quiz_color)
            screen.blit(quiz_text, (quiz_x + 35, learn_y))
