...
class AlphabitQuiz:
    ...
    # ----------------- Draw -----------------
    def draw_menu(self):
        title_surf = render_arabic_to_surface("اختر المستوى", ARABIC_FONT_PATH or "", 64, color=(0, 0, 0), bold=True, italic=True)
...
##        self.screen.blit(hint, hint.get_rect(center=(self.W//2, int(self.H*0.9))))
    ...
    def draw_playing(self):
        if not (0 <= self.current_idx < len(self.questions)): return
        total = len(self.questions)
        prog_surf = self.font_med.render(f"{self.current_idx+1}/{total}", True, (255,255,255))
