...
# ==========================================================
video_cache = {}
last_video_frame_cache = {} # Store the last frame of a video once it's finished
...
def run_learn_lvl1_T1(screen, main_app):
    ...
    slide_index = 0 # This will always be 0 as there's only one slide
    start_time = 0
    current_images = [] # Not used in single slide scenario
    playing_sound = False # Not explicitly used, handled by play_sound
    current_video_clip = None # Stores the moviepy VideoFileClip object
    video_start_time = 0 # Time when video playback started
    video_finished = False # Flag to indicate if video has finished playing
    last_rendered_video_frame_surface = None # Cache for the last rendered frame surface
    ...
    def render_slide(slide):
            ...
            if current_video_clip is None and not video_finished:
                ...
                if current_video_clip:
                    ...
                    if audio_path and os.path.exists(audio_path):
                        ...
                        try:
                            # Create and play temp audio file only if it doesn't exist or clip is new
                            if video_path not in last_video_frame_cache: # Using this as a proxy for 'new clip'
        ...
        # 3. Draw Foreground Images if present (on top of video/background) - Not expected for this specific slide
        if "images" in slide:
    ...
    # Initial slide setup (always the first slide)
    current_slide = slides[slide_index]
    if "narration" in current_slide:
        play_sound(current_slide["narration"])
    # Video audio will be handled by render_slide for "video" slides

    start_time = time.time() # This start_time is for image delays (though no images with delays here)

    while running:
        screen.fill((255, 255, 255))
        slide = slides[slide_index] # This will always be slides[0]
        ...
        # === Back Button (only navigation needed) ===
                ...
                if main_app.exit_rect.collidepoint(e.pos):
                    ...
                    if "video" in current_slide_data and current_slide_data["video"].get("audio_path"):
                        main_app.current_video_audio_path = current_slide_data["video"]["audio_path"]
                         ...
                         main_app.current_video_audio_path = None

                    stop_sounds()
                    main_app.open_profile_manager()

                    # Reset video state when returning from profile manager
                    if "video" in current_slide_data:
                        current_video_clip = None
                        video_start_time = 0
                        video_finished = False
                        last_rendered_video_frame_surface = None # Clear frame cache
                    else:
                        # Replay narration if not a video slide and returning from profile
        ...
        clock.tick(60) # Maintain a consistent frame rate for Pygame display
