...
class QuizApp:
    def __init__(self, selected_profile):
        ...
        self.current_video_audio_path = None
    ...
    def open_profile_manager(self):
        ...
        if updated:
            if updated.get("_deleted"):
                ...
                # After selecting a new profile, always play main BG music
                self.play_bg_music() 
            else:
                self.selected_profile.update(updated)
                avatar_path = os.path.join(config.AVATAR_PROFILES_PATH, self.selected_profile["avatar"])
                self.profile_icon = load_image(avatar_path, config.SCREEN_WIDTH * 0.04, config.SCREEN_WIDTH * 0.04)
                self.profile_rect = self.profile_icon.get_rect(topright=(config.SCREEN_WIDTH - 30, 30))
                
                # After returning from profile manager, decide which music to resume
                if self.current_video_audio_path:
                    # Resume video audio if it was playing
                    try:
                        pygame.mixer.music.load(self.current_video_audio_path)
                        pygame.mixer.music.play()
                        self.current_video_audio_path = None # Clear it after resuming
                    except Exception as e:
                        print(f"Error resuming video audio: {e}")
                        self.play_bg_music() # Fallback to BG music
                elif was_music_playing:
                    # Resume main background music if it was playing
                    self.play_bg_music()
                # Else, if nothing was playing, do nothing, let the calling screen decide.
