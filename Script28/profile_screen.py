...
pygame.init()
# NEW: Use load_font from screen_helpers and config paths for consistent font loading
...
# NEW: Use config paths directly
AVATAR_PATH = config.AVATAR_PROFILES_PATH
# DATA_PATH is now handled by profile_system itself using config.PROFILE_DATA_PATH
IMG_PATH = config.IMG_MAIN_PATH
EXIT_IMG = os.path.join(IMG_PATH, "exit_icon.png") # Still uses IMG_PATH here for now

# os.makedirs(DATA_PATH, exist_ok=True) # REMOVED: profile_system handles this
...
# ================== Profile Selection ==================
def run_profile_screen(screen, SCREEN_WIDTH_PARAM, SCREEN_HEIGHT_PARAM): # Use distinct param names
    """
    Startup selector:
    ...
    # UI rects - Use config.SCREEN_WIDTH/HEIGHT for dynamic scaling
    create_button = pygame.Rect(config.SCREEN_WIDTH * 0.4, config.SCREEN_HEIGHT * 0.85, config.SCREEN_WIDTH * 0.2, 60)
    name_rect = pygame.Rect(config.SCREEN_WIDTH // 2 - 180, 220, 360, 60)
    age_rect  = pygame.Rect(config.SCREEN_WIDTH // 2 - 180, 300, 360, 60)
    ...
    exit_rect = exit_icon.get_rect(bottomright=(config.SCREEN_WIDTH - 30, config.SCREEN_HEIGHT - 30))
    ...
    def draw_profiles():
        ...
        total_grid_height = rows * (CARD_H + 60) + (rows - 1) * MARGIN_Y

        # Center starting position
        start_x = (config.SCREEN_WIDTH - total_grid_width) // 2 + CARD_W // 2
        start_y = (config.SCREEN_HEIGHT - total_grid_height) // 2 + CARD_H // 2

        x, y = start_x, start_y

        if not profiles:
            info = FONT.render("No profiles found. Click 'New Profile' to create one.", True, (100,100,100))
            screen.blit(info, (config.SCREEN_WIDTH//2 - info.get_width()//2, config.SCREEN_HEIGHT//2))
        else:
            ...
            rows = (len(profiles) + cols - 1) // cols
            start_y = (config.SCREEN_HEIGHT - (rows * (CARD_H + MARGIN_Y))) // 2 + 150  # vertical center offset
            ...
            for row in range(rows):
                ...
                start_x = (config.SCREEN_WIDTH - total_row_width) // 2 + CARD_W // 2
    ...
    def draw_create_form():
        ...
        screen.blit(t, (config.SCREEN_WIDTH//2 - t.get_width()//2, 60))
...
##        screen.blit(avatars_title, (SCREEN_WIDTH // 2 - avatars_title.get_width() // 2, 400))
        ...
        AVATAR_SIZE = 120
        SPACING_X = 50
        y = int(config.SCREEN_HEIGHT * 0.7)  # vertical position of the row

        # dynamically center the row
        row_count = len(AVAILABLE_AVATARS)
        total_row_width = row_count * AVATAR_SIZE + (row_count - 1) * SPACING_X
        start_x = (config.SCREEN_WIDTH - total_row_width) // 2 + AVATAR_SIZE // 2
...
def run_profile_manage_screen(screen, SCREEN_WIDTH_PARAM, SCREEN_HEIGHT_PARAM, profile): # Use distinct param names
    """
    ...
    try:
        for r in AVAILABLE_REWARDS:
            path = os.path.join(config.REWARDS_CARDS_PATH, r) # Use config path here
    ...
    # --- UI Rects ---
    back_btn = pygame.Rect(80, config.SCREEN_HEIGHT - 100, 180, 60)
    edit_btn = pygame.Rect(config.SCREEN_WIDTH // 2 - 200, int(config.SCREEN_HEIGHT * 0.80), 160, 60)
    save_btn = pygame.Rect(config.SCREEN_WIDTH // 2 + 40, int(config.SCREEN_HEIGHT * 0.80), 160, 60)
    del_btn  = pygame.Rect(config.SCREEN_WIDTH - 260, config.SCREEN_HEIGHT - 100, 180, 60)
    ...
    BOX_W_RATIO  = 0.05    # width (25% of screen width) # Adjusted to be more reasonable
    BOX_H_RATIO  = 0.03    # height (5% of screen height) # Adjusted to be more reasonable

    # Create rects
    name_rect = pygame.Rect(
        int(config.SCREEN_WIDTH * NAME_X_RATIO),
        int(config.SCREEN_HEIGHT * NAME_Y_RATIO),
        int(config.SCREEN_WIDTH * BOX_W_RATIO),
        int(config.SCREEN_HEIGHT * BOX_H_RATIO)
    )

    age_rect = pygame.Rect(
        int(config.SCREEN_WIDTH * AGE_X_RATIO),
        int(config.SCREEN_HEIGHT * AGE_Y_RATIO),
        int(config.SCREEN_WIDTH * BOX_W_RATIO),
        int(config.SCREEN_HEIGHT * BOX_H_RATIO)
    )
    ...
    def draw():
        ...
        TITLE_X = int(config.SCREEN_WIDTH * PROFILE_TITLE_X_RATIO) # Use config
        TITLE_Y = int(config.SCREEN_HEIGHT * PROFILE_TITLE_Y_RATIO) # Use config
        ...
        avatar_size = int(config.SCREEN_WIDTH * AVATAR_SIZE_RATIO) # Use config
        x = int(config.SCREEN_WIDTH * AVATAR_START_X_RATIO) # Use config
        y = int(config.SCREEN_HEIGHT * AVATAR_START_Y_RATIO) # Use config
        ...
        for a in AVAILABLE_AVATARS:
            ...
            avatar_images[a + "_rect"] = r
            x += int(config.SCREEN_WIDTH * (AVATAR_SIZE_RATIO + AVATAR_GAP_RATIO)) # Use config  # move to next slot
        ...
        label_rect.right = name_rect.x - int(config.SCREEN_WIDTH * 0.01)   # 1% gap between label and box
        ...
        label_rect.right = age_rect.x - int(config.SCREEN_WIDTH * 0.01)
        screen.blit(label_surface, label_rect)
        ...
        title_x = int(config.SCREEN_WIDTH * ACH_TITLE_X_RATIO)
        title_y = int(config.SCREEN_HEIGHT * ACH_TITLE_Y_RATIO)
        screen.blit(reward_title, (title_x, title_y))


        # === Rewards Scroll Section (left-aligned) ===
        # Size
        view_width  = int(config.SCREEN_WIDTH * 0.37)
        view_height = int(config.SCREEN_HEIGHT * 0.38)
        reward_surface = pygame.Surface((view_width, view_height)) # view_width/height should also use config
        reward_surface.fill((235, 255, 235))

        cols = 7
        card_width = int(config.SCREEN_WIDTH * 0.05) # Use config
        card_height = int(config.SCREEN_HEIGHT * 0.18)
        margin_x = 10
        margin_y = 20
        reward_rects = {}
        ...
        for i, (r, card_img) in enumerate(reward_cards):
            ...
            if r not in working.get("rewards", []):
                ...
                abs_x = x + int(config.SCREEN_WIDTH * 0.01)   # ← matches your screen.blit() X offset
                abs_y = y + int(config.SCREEN_HEIGHT * 0.35)  # ← matches your screen.blit() Y offset
                reward_rects[r] = pygame.Rect(abs_x, abs_y, card_width, card_height)


            reward_surface.blit(img, (x, y))

        screen.blit(reward_surface, (int(config.SCREEN_WIDTH * 0.01), int(config.SCREEN_HEIGHT * 0.35)))
...
##        screen.blit(progress_title, (SCREEN_WIDTH // 2 - progress_title.get_width() // 2, progress_y))
        ...
        bar_x = int(config.SCREEN_WIDTH * PROG_X_RATIO)
        bar_y = int(config.SCREEN_HEIGHT * PROG_Y_RATIO)
        bar_w = int(config.SCREEN_WIDTH * PROG_W_RATIO)
        bar_h = int(config.SCREEN_HEIGHT * PROG_H_RATIO)
        ...
        info_rect = info_text.get_rect(midleft=(bar_x, bar_y + bar_h + int(config.SCREEN_HEIGHT * 0.03)))
    ...
    while True:
        ...
        if zoomed_reward:
            ...
            for r, img in reward_cards:
                if r == zoomed_reward:
                    large_w = int(config.SCREEN_WIDTH * 0.20)
                    large_h = int(SCREEN_HEIGHT * 0.72)
                    large_img = pygame.transform.smoothscale(img, (large_w, large_h))
                    rect = large_img.get_rect(center=(config.SCREEN_WIDTH // 2, config.SCREEN_HEIGHT // 2))
                    screen.blit(large_img, rect)
                    break
            hint = FONT.render("Click anywhere to close", True, (255, 255, 255))
            screen.blit(hint, (config.SCREEN_WIDTH // 2 - hint.get_width() // 2, config.SCREEN_HEIGHT - 80))
