...
def run_profile_manage_screen(screen, SCREEN_WIDTH, SCREEN_HEIGHT, profile):
    ...
    Manage current profile from main menu:
      ...
      - Delete profile
      - View rewards (unlocked visible, locked greyed)
      - Click on unlocked reward to zoom in/out
    """
    clock = pygame.time.Clock()
    avatar_images = _load_avatar_images()
    ...
    def draw():
        ...
        screen.blit(reward_title, (SCREEN_WIDTH // 2 - reward_title.get_width() // 2, int(SCREEN_HEIGHT * 0.43)))

        card_width = int(SCREEN_WIDTH * 0.05)
        card_height = int(SCREEN_HEIGHT * 0.18)
        reward_spacing = int(card_width * 1.3)
        reward_y = int(SCREEN_HEIGHT * 0.49)
        total_width = len(reward_cards) * reward_spacing
        reward_x = SCREEN_WIDTH // 2 - total_width // 2
        ...
        for r, card_img in reward_cards:
            ...
            if r not in working.get("rewards", []):
                ...
                reward_rects[r] = pygame.Rect(reward_x, reward_y, card_width, card_height)

            screen.blit(img, (reward_x, reward_y))
            reward_x += reward_spacing
    ...
    while True:
        ...
        for e in pygame.event.get():
            if e.type == pygame.QUIT:
                ...
                # Edit
                elif edit_btn.collidepoint(mouse):
                    editing = True
                    active_input = "name"
                ...
                # Input focus
                    ...
                    if name_rect.collidepoint(mouse):
                        active_input = "name"
                    elif age_rect.collidepoint(mouse):
                        active_input = "age"
