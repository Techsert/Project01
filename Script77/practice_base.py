        ...
        for event in pygame.event.get():
                ...
                if self.state == "playing":
                    ...
                    if q_type == "drag_drop" and self.dragging:
                        ...
                        if self.tracing_active:
                            ...
                            # Check if user reached the LAST guide point (must complete the full path)
                            if hasattr(self, 'last_matched_index') and self.last_matched_index >= len(self.trace_path):
                                self.trace_complete = True
                                self.check_answer()
                            else:
                                # Didn't reach the end, play try again
                                self._play_tryagain_sound()
                                # Reset for retry
                                self.trace_points = []
                                self.trace_progress = 0.0
            ...
            # ✅ Handle mouse motion for dragging and tracing
                ...
                if self.state == "playing" and self.dragging and self.dragged_item:
                    ...
                    self.dragged_item["rect"].x = mouse_pos[0] + self.drag_offset[0]
                    self.dragged_item["rect"].y = mouse_pos[1] + self.drag_offset[1]
                    ...
                    if self.trace_path and q.get("letter_offset"):
                        ...
                        # Check if we've reached the next guide point in sequence
                        if self.last_matched_index < len(self.trace_path):
                            ...
                            # Check if current mouse position is close to next guide point
                            dx = mouse_pos[0] - adjusted_guide[0]
                            dy = mouse_pos[1] - adjusted_guide[1]
                            distance = (dx * dx + dy * dy) ** 0.5
                            
                            if distance < self.trace_tolerance:
                                # Matched next point in sequence!
                                self.last_matched_index += 1
                        
                        # Update progress based on sequential matches
                        self.trace_progress = min(1.0, self.last_matched_index / max(1, len(self.trace_path)))
