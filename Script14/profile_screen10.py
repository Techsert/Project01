...
def run_profile_manage_screen(screen, SCREEN_WIDTH, SCREEN_HEIGHT, profile):
    ...
    # --- Load reward cards once at start ---
    reward_cards = []
    try:
    ...
    def draw():
        ...
        screen.blit(
            reward_title,
            (SCREEN_WIDTH // 2 - reward_title.get_width() // 2, int(SCREEN_HEIGHT * 0.58))
        )

        # --- Responsive sizing ---
        card_width = int(SCREEN_WIDTH * 0.04)    # 10% of screen width
        card_height = int(SCREEN_HEIGHT * 0.18)  # 18% of screen height
        reward_spacing = int(card_width * 1.3)   # horizontal spacing based on card width
        reward_y = int(SCREEN_HEIGHT * 0.65)
        ...
        for r, card_img in reward_cards:
            ...
            if r not in working.get("rewards", []):
                # Convert to grayscale for locked rewards
                arr = pygame.surfarray.array3d(img)
                avg = arr.mean(axis=2, keepdims=True)
                arr[:] = avg
                img = pygame.surfarray.make_surface(arr)
                img.set_alpha(120)  # make it semi-transparent
