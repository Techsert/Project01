...
def draw_quiz_screen(screen, main_app, title, buttons, bg_image, bg_rect, level=1):
    """
    ✅ FIXED: Draws a quiz topic screen with IMAGE ICONS instead of emojis
    """
                    ...
                    for i, b in enumerate(buttons):
                        if b["rect"].collidepoint(e.pos):
                            ...
                            if is_unlocked:
                                ...
                                msg = f"Complete all Learning Level {level}\ntopics first to unlock Quiz Level {level}!"
...
def quiz_one(screen, main_app):
    ...
    topics_data = [
        ("Quiz LVL 1", main_app.launch_quiz_lvl1, "hover_quiz_lvl_1.ogg", "Q_lvl1.png"),
    ]
...
def quiz_two(screen, main_app):
    ...
    # New structure for topics: (text, action, hover_sound, button_image_filename)
    topics_data = [
        ("Quiz LVL 2", main_app.launch_quiz_lvl2, "hover_Lvl1_Topic1_ogg.ogg", "Q_lvl2.png"),
    ]
