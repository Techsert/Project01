...
class WhatIsThis:
    ...
    def preload_sounds(self, questions):
        for q in questions:
            paths = [q["sound"]] + ["correct.wav", "wrong.wav"]
            for s in paths:
    ...
    def show_feedback(self, is_correct=True):
        ...
        if sound_file in self.loaded_sounds:
            self.loaded_sounds[sound_file].play()

        # Function to close popup and go to next question
        def close_popup(event=None):
            try:
                popup.after_cancel(auto_close_id)  # cancel timer if still running
            except:
                pass
            popup.destroy()
            self.buttons_disabled = False
        ...
        # Bind click on feedback image to skip immediately
        label_img.bind("<Button-1>", close_popup)

        # Auto-close popup after 2 seconds
        auto_close_id = popup.after(6000, close_popup)
...
##            self.show_final_result()
    ...
    def play_answer_sound(self, index):
        ...
        # Directly check the answer without playing any sound
        self.root.after(200, lambda: self.check_answer(index))
