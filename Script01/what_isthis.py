...
class WhatIsThis:
    ...
    def show_feedback(self, title, message, is_correct=True):
        ...
        popup.configure(bg="#ccffcc" if is_correct else "#ffcccc")
        ...
        label = tk.Label(
            ...
            bg=popup["bg"],
            fg="#006600" if is_correct else "#990000",
            wraplength=450,
            justify="center"
        )
        label.pack(pady=30)

        # ✅ OK button triggers next question
        ok_btn = tk.Button(
            popup,
            text="موافق",
            font=("Scheherazade", 18, "bold"),
            bg="#ffffff",
            command=lambda: self.close_feedback_and_continue(popup)
        )
        ok_btn.pack(pady=10)
