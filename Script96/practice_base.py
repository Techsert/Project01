...
##            self.exit_rect = self.exit_img.get_rect(center=(self.W // 2, self.H - 80))
    ...
    def draw_drag_drop(self):
        ...
        # ==========================================
        # 🛠️ DEBUG TOOL: COORDINATE FINDER (SAME AS TRACING)
        # =======================================================
        mx, my = pygame.mouse.get_pos()
        
        # For drag-drop, you might want to show coordinates relative to screen
        # Or relative to a drop zone if mouse is over one
        try:
            ...
            # Text 2: Check if mouse is over a zone
            over_zone = None
            for zone in zones:
                if "rect" in zone and zone["rect"].collidepoint((mx, my)):
                    over_zone = zone
                    break
            
            if over_zone:
                # Show relative coordinates to zone center
                zone_center_x = over_zone["rect"].centerx
                zone_center_y = over_zone["rect"].centery
                rel_x = mx - zone_center_x
                rel_y = my - zone_center_y
                txt_code = font.render(f"Zone '{over_zone.get('label', 'N/A')}' Offset: ({rel_x}, {rel_y})", True, (255, 0, 0), (255, 255, 255))
            else:
                # Show center-relative coordinates for item positioning
                center_x = self.W // 2
                center_y = self.H // 2
                rel_x = mx - center_x
                rel_y = my - center_y
                txt_code = font.render(f"Center Offset: ({rel_x}, {rel_y})", True, (255, 0, 0), (255, 255, 255))
