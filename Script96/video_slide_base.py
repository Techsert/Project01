...
class VideoSlidePlayer:
    ...
    def handle_mouse_click(self, pos, is_video_playing):
        ...
        if self.main_app.profile_rect.collidepoint(pos):
...
class NavigableVideoSlidePlayer(VideoSlidePlayer):
    ...
    def draw_navigation_buttons(self, is_video_playing):
        ...
        button_width = 250
        button_height = 100
        y_position = int(self.SCREEN_HEIGHT * 0.90)
        
        # ✅ Determine which buttons to show
        show_prev = self.slide_index > 0  # Show Prev if not on first slide
        show_next = self.slide_index < len(self.slides) - 1  # Show Next if not on last slide
        
        # ✅ Calculate positions based on which buttons are visible
        buttons_to_show = []
        if show_prev:
            buttons_to_show.append('prev')
        buttons_to_show.append('back')  # Back button always shows
        if show_next:
            buttons_to_show.append('next')
        
        num_buttons = len(buttons_to_show)
        total_width = self.SCREEN_WIDTH
        spacing = (total_width - (num_buttons * button_width)) / (num_buttons + 1)
        
        # ✅ Position buttons dynamically
        current_x = spacing
        
        # ✅ NEXT button on the LEFT (for RTL/Arabic)
        if show_next:
            self.next_rect = pygame.Rect(int(current_x), y_position, button_width, button_height)
        ...
        # Back button (always shown, in center)
        self.back_rect = pygame.Rect(int(current_x), y_position, button_width, button_height)
        back_color = (200, 80, 80)
        pygame.draw.rect(self.screen, back_color, self.back_rect, border_radius=15)
        self.main_app.draw_arabic_text("Finish", 0.035, (255, 255, 255), self.back_rect.center)
        current_x += button_width + spacing
        
        # ✅ PREV button on the RIGHT (for RTL/Arabic)
        if show_prev:
            self.prev_rect = pygame.Rect(int(current_x), y_position, button_width, button_height)
    ...
    def handle_mouse_click(self, pos, is_video_playing):
        ...
        if self.main_app.profile_rect.collidepoint(pos):
            ...
            self.handle_back_click()
        elif self.next_rect and self.next_rect.collidepoint(pos) and not is_video_playing:  # ✅ Check if next_rect exists
            self.navigate_to_next_slide()
        elif self.prev_rect and self.prev_rect.collidepoint(pos) and not is_video_playing:  # ✅ Check if prev_rect exists
