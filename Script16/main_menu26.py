...
##            pygame.time.set_timer(pygame.USEREVENT + 1, delay)
    ...
    def open_learning_screen(self):
        """Learning screen: shows soccer field with LVL 1–9 buttons scattered around."""
        ...
        # === Adjustable scatter positions (X, Y ratios) ===
        # Each LVL button has its own (x_ratio, y_ratio)
        scatter_positions = [
            (0.10, 0.40),  # LVL 1 - top left
            (0.19, 0.85),  # LVL 2 - near top center
            (0.29, 0.15),  # LVL 3 - top right
            (0.39, 0.67),  # LVL 4 - mid left
            (0.49, 0.50),  # LVL 5 - center
            (0.59, 0.67),  # LVL 6 - mid right
            (0.69, 0.15),  # LVL 7 - bottom left
            (0.79, 0.38),  # LVL 8 - bottom center
            (0.89, 0.50),  # LVL 9 - bottom right
        ]
        ...
        while running:
            ...
            # === Page Title ===
            title_font = dynamic_font(0.04, bold=True)  # scales with screen height
            title_text = title_font.render("Learning Levels", True, (255, 255, 255))  # title color
            title_rect = title_text.get_rect(center=(SCREEN_WIDTH // 2, int(SCREEN_HEIGHT * 0.04)))
            ...
            # Draw scattered LVL buttons
            img_w = int(SCREEN_WIDTH * IMG_W_RATIO)
            img_h = int(SCREEN_HEIGHT * IMG_H_RATIO)
            font = dynamic_font(0.02, bold=True)
            ...
            for (x_ratio, y_ratio), btn in zip(scatter_positions, lvl_buttons):
                ...
                label_rect = label.get_rect(center=(x, y + img_h // 2 + 30))
                screen.blit(label, label_rect)

            # Hint text
            # Hint text (interactive BACK button)
            hint_font = dynamic_font(0.03, bold=True)
    ...
    def open_quiz_screen(self):
        """Quiz screen: identical layout to Learning screen but with title 'Quizzes'."""
        soccer_path = os.path.join(IMG_PATH, "soccerfield.png")
        ...
        # === Same scatter positions as Learning ===
        scatter_positions = [
            (0.10, 0.40),
            (0.19, 0.85),
            (0.29, 0.15),
        ...
        while running:
            ...
            for e in pygame.event.get():
                if e.type == pygame.QUIT:
                    ...
                    if self.profile_rect.collidepoint(e.pos):
                        ...
                        for b in lvl_buttons:
                            if b["rect"] and b["rect"].collidepoint(e.pos):
                                print(f"Clicked {b['label']}!")  # placeholder
                                break
            ...
            # === Page Title ===
            title_font = dynamic_font(0.04, bold=True)
