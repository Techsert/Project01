...
class PracticeBase:
        ...
        if q_type == "drag_drop":
            ...
            for item in items:
                ...
                if img:
                    ...
                    # ✅ FIXED: Convert relative positions to absolute screen positions
                    x_offset = item.get("x", 0)
                    y_offset = item.get("y", 0)
                    
                    # If negative or small values, treat as offset from center
    ...
    def start_practice(self):
        ...
        # ✅ FIXED: Load assets for first question BEFORE changing state
        if self.questions:
...
# ==========================================================
    ...
    def draw_drag_drop(self):
        ...
        if len(self.placed_items) == len(items):
            btn_rect = pygame.Rect(self.W // 2 - 100, self.H - 150, 200, 60)
    ...
    def draw_tracing(self):
        ...
        if self.trace_path:
            ...
            for i, point in enumerate(self.trace_path):
                ...
                color = (0, 255, 0) if i < len(self.trace_points) else (200, 200, 200)
                pygame.draw.circle(self.screen, color, adjusted_point, 8)
    ...
    def handle_events(self):
        ...
        for event in pygame.event.get():
            ...
            # ✅ FIXED: Handle mouse release for drag-drop
            ...
            # ✅ FIXED: Handle mouse release for drag-drop AND tracing
                ...
                if self.state == "playing":
                    ...
                    if q_type == "drag_drop" and self.dragging:
                        ...
                        # Stop tracing regardless of state
                        if self.tracing_active:
                            self.tracing_active = False
                            
                            # Check if trace is complete enough
                            if self.trace_progress >= 0.7:  # 70% completion threshold
            ...
            # ✅ Handle mouse motion for dragging and tracing
                ...
                if self.state == "playing" and self.dragging and self.dragged_item:
                    # Move dragged item with mouse
                    self.dragged_item["rect"].x = event.pos[0] + self.drag_offset[0]
                    self.dragged_item["rect"].y = event.pos[1] + self.drag_offset[1]
                
                elif self.state == "playing" and self.tracing_active:
                    q = self.questions[self.current_idx]
                    
                    # Only add points while actively tracing (mouse is down and moving)
                    if len(self.trace_points) > 0:  # Safety check
                        self.trace_points.append(event.pos)
                        
                        # Calculate progress by checking proximity to guide points
                        if self.trace_path and q.get("letter_offset"):
                            letter_offset = q["letter_offset"]
                            matched_points = 0
                            
                            for guide_point in self.trace_path:
                                # Adjust guide point to screen position
                                adjusted_guide = (
                                    guide_point[0] + letter_offset[0],
                                    guide_point[1] + letter_offset[1]
                                )
                                
                                # Check if any traced point is close to this guide point
                                for traced_point in self.trace_points:
                                    dx = traced_point[0] - adjusted_guide[0]
                                    dy = traced_point[1] - adjusted_guide[1]
                                    distance = (dx * dx + dy * dy) ** 0.5
                                    
                                    if distance < self.trace_tolerance:
                                        matched_points += 1
                                        break
                            
                            # Update progress
                            self.trace_progress = min(1.0, matched_points / max(1, len(self.trace_path)))
