...
def run_profile_screen(screen, SCREEN_WIDTH, SCREEN_HEIGHT):
    ...
    back_button = pygame.Rect(100, SCREEN_HEIGHT - 100, 200, 60)
    edit_button = pygame.Rect(SCREEN_WIDTH // 2 - 150, SCREEN_HEIGHT * 0.8, 120, 60)
    save_button = pygame.Rect(SCREEN_WIDTH // 2 + 50, SCREEN_HEIGHT * 0.8, 120, 60)
    ...
    def draw_profile_view(profile):
        screen.fill((245, 245, 245))
        title = TITLE_FONT.render("Profile Details", True, (0, 0, 0))
        ...
        avatar = avatar_images.get(avatar_name)
        if avatar:
            avatar = pygame.transform.smoothscale(avatar, (200, 200))
            rect = avatar.get_rect(center=(SCREEN_WIDTH // 2, 250))
            screen.blit(avatar, rect)

        # Name & Age
        name_text = FONT.render(f"Name: {profile.get('name', '')}", True, (0, 0, 0))
        age_text = FONT.render(f"Age: {profile.get('age', '')}", True, (0, 0, 0))
        screen.blit(name_text, (SCREEN_WIDTH // 2 - name_text.get_width() // 2, 480))
        screen.blit(age_text, (SCREEN_WIDTH // 2 - age_text.get_width() // 2, 540))

        # Buttons
        pygame.draw.rect(screen, (255, 102, 0), edit_button, border_radius=15)
        pygame.draw.rect(screen, (0, 200, 0), save_button, border_radius=15)
        pygame.draw.rect(screen, (200, 0, 0), back_button, border_radius=15)

        screen.blit(FONT.render("Edit", True, (255, 255, 255)), edit_button.move(25, 10))
        screen.blit(FONT.render("Save", True, (255, 255, 255)), save_button.move(25, 10))
        screen.blit(FONT.render("Back", True, (255, 255, 255)), back_button.move(50, 10))
    ...
    while True:
            ...
            if event.type == pygame.QUIT:
                ...
                if mode == "select":
                    ...
                    if back_button.collidepoint(mouse_pos):
                        mode = "select"
                        selected_profile = None
                    elif edit_button.collidepoint(mouse_pos):
                        is_editing = True
                        current_input["name"] = selected_profile.get("name", "")
                        current_input["age"] = selected_profile.get("age", "")
                    elif save_button.collidepoint(mouse_pos) and is_editing:
                        selected_profile["name"] = current_input["name"]
                        selected_profile["age"] = current_input["age"]
                        clean_profiles = [
                            {k: v for k, v in p.items() if k not in ("rect", "delete_rect")}
                            for p in profiles
                        ]
        ...
        if mode == "select":
            ...
            draw_create_form()
        elif mode == "view" and selected_profile:
            draw_profile_view(selected_profile)
        else:
            draw_create_form()
