...
from profile_screen08 import run_profile_screen
...
class QuizApp:
    def __init__(self, selected_profile):
        ...
        avatar_filename = self.selected_profile.get("avatar", None) if self.selected_profile else None
        ...
        if avatar_filename:
            ...
            if os.path.exists(avatar_path):
                ...
                self.profile_icon = load_image(os.path.join(IMG_PATH, "profile_icon.png"), 100, 100)
        else:
            self.profile_icon = load_image(os.path.join(IMG_PATH, "profile_icon.png"), 100, 100)
    ...
    def open_profile_page(self):
        """Open the user profile screen and update avatar if changed."""
        print("Opening profile screen...")
        pygame.mixer.music.stop()

        updated_profile = run_profile_screen(screen, SCREEN_WIDTH, SCREEN_HEIGHT)
        if updated_profile:
            self.selected_profile = updated_profile
            # Update avatar if changed
            avatar_folder = os.path.join(BASE_DIR, "../assets/profiles/avatars")
            avatar_filename = self.selected_profile.get("avatar", None)
            if avatar_filename:
                avatar_path = os.path.join(avatar_folder, avatar_filename)
                if os.path.exists(avatar_path):
                    self.profile_icon = load_image(avatar_path, 100, 100)
    ...
    def run(self):
        while self.running:
            ...
            # Draw profile avatar (always visible)
