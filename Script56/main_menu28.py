...
class QuizApp:
    ...
    def _reload_profile(self):
        ...
        from profile_system import load_profiles, check_and_unlock_rewards
        ...
        for p in profiles:
            if p.get("name") == current_name:
                # ✅ Check for newly unlocked rewards and add to popup
                newly_unlocked = check_and_unlock_rewards(p, reward_popup=self.reward_popup)
                
                # Update the selected profile with fresh data
                self.selected_profile.update(p)
                print(f"🔄 Reloaded profile: {current_name}, Rewards: {len(self.selected_profile.get('rewards', []))}")
                
                # ✅ Show reward popup if there are newly unlocked rewards
                if newly_unlocked and self.reward_popup.has_pending_rewards():
