# profile_system.py - UPDATED WITH REWARD POPUP SUPPORT
import os, json
import config 
...
REWARD_CRITERIA = {
    ...
    # ===== QUIZ PERFECT SCORES =====
    "reward_13.png": {"type": "quiz_perfect", "level": 1},
    "reward_14.png": {"type": "quiz_perfect", "level": 2},
    "reward_15.png": {"type": "quiz_perfect", "level": 3},
    
    # ===== QUIZ GOOD SCORES =====
    "reward_16.png": {"type": "quiz_good", "level": 1, "min_score": 90},
    "reward_17.png": {"type": "quiz_good", "level": 2, "min_score": 90},
    "reward_18.png": {"type": "quiz_good", "level": 3, "min_score": 90},
    
    # ===== ACHIEVEMENT REWARDS =====
    "reward_19.png": {"type": "all_learning", "levels": [1, 2, 3]},
    "reward_20.png": {"type": "all_quiz", "levels": [1, 2, 3], "min_score": 90},
}
...
def mark_learning_topic_complete(profile_name, level, topic):
    ...
    ✅ FIXED: Does NOT check rewards here - let main app handle it
    """
    profiles = load_profiles()
    for p in profiles:
        if p.get("name") == profile_name:
            ...
            # ✅ REMOVED: check_and_unlock_rewards(p)
...
# ==========================================================
# ✅ UPDATED: REWARDS SYSTEM WITH POPUP SUPPORT
# ==========================================================
def check_and_unlock_rewards(profile, reward_popup=None):
    ...
    print(f"   Current rewards: {len(current_rewards)}")
    ...
    for reward_file, criteria in REWARD_CRITERIA.items():
        ...
        # ✅ NEW: Individual topic completion
        if criteria["type"] == "topic_complete":
            ...
            if level in quiz:
                ...
                if unlocked:
                    print(f"   ✅ Unlocking {reward_file}: Perfect score on Quiz Level {level}")
            ...
            if level in quiz:
                ...
                if unlocked:
                    print(f"   ✅ Unlocking {reward_file}: {min_score}%+ on Quiz Level {level}")
            ...
            if unlocked:
                print(f"   ✅ Unlocking {reward_file}: All learning levels complete")
            ...
            if unlocked:
                print(f"   ✅ Unlocking {reward_file}: All quizzes {min_score}%+")
        
        if unlocked:
            newly_unlocked.append(reward_file)
            current_rewards.add(reward_file)
            
            # ✅ Add to reward popup if provided
            if reward_popup:
    ...
    if newly_unlocked:
        ...
        print(f"   No new rewards unlocked")
